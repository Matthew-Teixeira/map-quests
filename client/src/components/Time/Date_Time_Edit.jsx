...
export const Date_Time_Edit = ({ token, timeData }) => {
  ...
  useEffect(() => {
    setTimeId(window.location.pathname.split("/")[2]);

    timeData.start_time = DateTime.fromISO(timeData.start_time).toISO();
    timeData.end_time = DateTime.fromISO(timeData.end_time).toISO();
    ...
    setDateTimeParams({
      start_time: timeData.start_time.slice(0, 16),
      end_time: timeData.end_time.slice(0, 16),
      min: min.slice(0, 16),
      max: max.slice(0, 16)
    });

    setdateTimeEnd({ end_time: timeData.end_time.slice(0, 16) });
    setdateTimeStart({ start_time: timeData.start_time.slice(0, 16) });
  }, []);
  ...
  const time_sub = async (start_time, end_time) => {
    try {
      ...
      if (response.ok) {
        window.location.assign("/my_time");
      } else {
  ...
  return (
    <div className="relative flex px-2 flex-col justify-center h-[calc(100vh-66px)] overflow-hidden">
      <div className="mx-auto w-full px-2 bg-slate-400 rounded-md shadow-md lg:max-w-xl dark:bg-gray-800">
        {dateTimeParams ? (
          <form onSubmit={submitTime}>
            <div className="my-4 px-2 pb-4 bg-slate-200 flex flex-col justify-center rounded-sm dark:bg-slate-500">
              ...
              <input
                ...
                id="start_time"
                name="start_time"
                defaultValue={dateTimeParams.start_time}
            ...
            <div className="mb-8 px-2 pb-4 mt-8 bg-slate-200 flex flex-col justify-center rounded-sm dark:bg-slate-500">
              ...
              <input
                ...
                id="end_time"
                name="end_time"
                defaultValue={dateTimeParams.end_time}
                min={dateTimeParams.min}
                max={dateTimeParams.max}
